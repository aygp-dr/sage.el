#+TITLE: Rate Limiting in sage
#+AUTHOR: Jason Walsh
#+DATE: 2024-12-23

* Overview

The =sage-ratelimit= module provides automatic rate limiting for API requests to prevent exceeding provider limits and potential service interruptions or account penalties.
...
Configure via:
#+begin_src elisp
(setq sage-ratelimit-safety-margin 0.8)  ; 80% instead of 90%
#+end_src
...
Rate limiting is automatically applied to all API requests in =sage=. No configuration needed.
...
#+begin_src elisp
(if (sage-ratelimit-check "gemini-2.0-flash-exp")
...
#+begin_src elisp
(sage-ratelimit-record "gemini-2.0-flash-exp")
#+end_src
...
(sage-ratelimit-wait "gemini-2.0-flash-exp")
#+end_src
...
;; Programmatic
(sage-ratelimit-status "gemini-2.0-flash-exp")
#+end_src
...
#+begin_src elisp
(sage-ratelimit-enable-modeline "gemini-2.0-flash-exp")
#+end_src
...
(sage-ratelimit-configure-limit "my-custom-model" 20)  ; 20 RPM

;; Set unlimited
(sage-ratelimit-configure-limit "my-local-model" nil)
#+end_src
...
#+begin_src elisp
(setq sage-ratelimit-show-countdown nil)
#+end_src
...
#+begin_src elisp
(setq sage-ratelimit-window 120)  ; 2 minutes
#+end_src

* Testing

The module includes comprehensive tests in =test/sage-ratelimit-test.el=.
...
1. User makes request via =sage--request=
2. Request wrapped by =sage-ratelimit-wrap-request=
3. Check if allowed via =sage-ratelimit-check=
4. If not allowed, wait via =sage-ratelimit-wait=
5. Record request via =sage-ratelimit-record=
...
(sage-ratelimit-configure-limit "claude-3-opus" 5)
...
;; Check status
(sage-ratelimit-status "claude-3-opus")
#+end_src
...
- Ensure =sage-ratelimit-enable-modeline= was called
