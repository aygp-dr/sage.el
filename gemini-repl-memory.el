;;; gemini-repl-memory.el --- Persistent memory/facts system for gemini-repl -*- lexical-binding: t; -*-

;; Copyright (C) 2024 Jason Walsh
;; Author: Jason Walsh <j@wal.sh>
;; Version: 0.1.0
;; Package-Requires: ((emacs "28.1"))
;; Keywords: ai, tools, memory
;; URL: https://github.com/aygp-dr/gemini-repl-010

;; This file is not part of GNU Emacs.

;;; Commentary:

;; gemini-repl-memory provides a persistent memory/facts system for the
;; gemini-repl package.  It allows storing and retrieving facts across
;; sessions, categorizing them, and including them in conversation context.
;;
;; Features:
;; - Persistent JSON storage of facts
;; - Categorization (general, preference, project, technical)
;; - Automatic timestamping
;; - Context generation for LLM prompts
;; - Query and filtering by category
;;
;; Usage:
;;   (gemini-repl-memory-add "user-name" "Jason" 'preference)
;;   (gemini-repl-memory-get "user-name")
;;   (gemini-repl-memory-list 'project)
;;   (gemini-repl-memory-to-context)
;;   (gemini-repl-memory-remove "user-name")
;;   (gemini-repl-memory-clear)

;;; Code:

(require 'json)
(require 'cl-lib)

;;; Customization

(defgroup gemini-repl-memory nil
  "Persistent memory/facts system for gemini-repl."
  :group 'gemini-repl
  :prefix "gemini-repl-memory-")

(defcustom gemini-repl-memory-file
  (expand-file-name "gemini-repl/memory/facts.json" user-emacs-directory)
  "File to store persistent facts."
  :type 'file
  :group 'gemini-repl-memory)

(defcustom gemini-repl-memory-auto-load t
  "When non-nil, automatically load facts on first access."
  :type 'boolean
  :group 'gemini-repl-memory)

(defcustom gemini-repl-memory-auto-save t
  "When non-nil, automatically save facts after modifications."
  :type 'boolean
  :group 'gemini-repl-memory)

;;; Variables

(defvar gemini-repl-memory--facts nil
  "In-memory storage of facts.
Alist of (KEY . FACT) where FACT is a plist with:
  :key       - unique identifier
  :value     - fact value
  :category  - one of general, preference, project, technical
  :timestamp - ISO 8601 timestamp")

(defvar gemini-repl-memory--loaded nil
  "Whether facts have been loaded from disk.")

(defvar gemini-repl-memory-categories
  '(general preference project technical)
  "Valid fact categories.")

;;; Core Functions

(defun gemini-repl-memory--ensure-loaded ()
  "Ensure facts are loaded from disk."
  (when (and gemini-repl-memory-auto-load
             (not gemini-repl-memory--loaded))
    (gemini-repl-memory-load)))

(defun gemini-repl-memory--ensure-directory ()
  "Ensure memory directory exists."
  (let ((dir (file-name-directory gemini-repl-memory-file)))
    (unless (file-directory-p dir)
      (make-directory dir t))))

(defun gemini-repl-memory--validate-category (category)
  "Validate CATEGORY, return symbol or signal error."
  (let ((cat-symbol (if (stringp category)
                        (intern category)
                      category)))
    (unless (memq cat-symbol gemini-repl-memory-categories)
      (error "Invalid category: %s. Must be one of: %s"
             category gemini-repl-memory-categories))
    cat-symbol))

(defun gemini-repl-memory--timestamp ()
  "Return current timestamp in ISO 8601 format."
  (format-time-string "%Y-%m-%dT%H:%M:%S%z"))

(defun gemini-repl-memory--create-fact (key value category)
  "Create a fact plist.
KEY is the unique identifier.
VALUE is the fact value.
CATEGORY is the fact category."
  (list :key key
        :value value
        :category (gemini-repl-memory--validate-category category)
        :timestamp (gemini-repl-memory--timestamp)))

;;; Public API

;;;###autoload
(defun gemini-repl-memory-add (key value &optional category)
  "Add or update a fact.
KEY is a unique identifier (string).
VALUE is the fact value (string).
CATEGORY is one of general, preference, project, technical.
Defaults to general."
  (interactive
   (let* ((key (read-string "Fact key: "))
          (value (read-string "Fact value: "))
          (category (intern (completing-read
                            "Category: "
                            gemini-repl-memory-categories
                            nil t nil nil "general"))))
     (list key value category)))
  (gemini-repl-memory--ensure-loaded)
  (let* ((category (or category 'general))
         (fact (gemini-repl-memory--create-fact key value category)))
    ;; Remove existing fact with same key if present
    (setq gemini-repl-memory--facts
          (assoc-delete-all key gemini-repl-memory--facts))
    ;; Add new fact
    (push (cons key fact) gemini-repl-memory--facts)
    ;; Auto-save if enabled
    (when gemini-repl-memory-auto-save
      (gemini-repl-memory-save))
    fact))

;;;###autoload
(defun gemini-repl-memory-get (key)
  "Get fact by KEY.
Returns the fact plist or nil if not found."
  (interactive "sFact key: ")
  (gemini-repl-memory--ensure-loaded)
  (let ((fact (alist-get key gemini-repl-memory--facts nil nil #'string=)))
    (when (called-interactively-p 'any)
      (if fact
          (message "%s: %s [%s]"
                   key
                   (plist-get fact :value)
                   (plist-get fact :category))
        (message "Fact not found: %s" key)))
    fact))

;;;###autoload
(defun gemini-repl-memory-remove (key)
  "Remove fact by KEY.
Returns t if fact was removed, nil if not found."
  (interactive
   (list (completing-read "Remove fact: "
                         (mapcar #'car gemini-repl-memory--facts)
                         nil t)))
  (gemini-repl-memory--ensure-loaded)
  (let ((found (assoc key gemini-repl-memory--facts)))
    (when found
      (setq gemini-repl-memory--facts
            (assoc-delete-all key gemini-repl-memory--facts))
      (when gemini-repl-memory-auto-save
        (gemini-repl-memory-save))
      (when (called-interactively-p 'any)
        (message "Removed fact: %s" key))
      t)))

;;;###autoload
(defun gemini-repl-memory-list (&optional category)
  "List all facts, optionally filtered by CATEGORY.
Returns list of fact plists."
  (interactive
   (list (when current-prefix-arg
           (intern (completing-read "Category (empty for all): "
                                   gemini-repl-memory-categories
                                   nil t)))))
  (gemini-repl-memory--ensure-loaded)
  (let ((facts (if category
                   (cl-remove-if-not
                    (lambda (fact)
                      (eq (plist-get (cdr fact) :category)
                          (gemini-repl-memory--validate-category category)))
                    gemini-repl-memory--facts)
                 gemini-repl-memory--facts)))
    (when (called-interactively-p 'any)
      (if facts
          (let ((buf (get-buffer-create "*Gemini REPL Memory*")))
            (with-current-buffer buf
              (erase-buffer)
              (insert (format "Gemini REPL Memory Facts%s\n\n"
                            (if category
                                (format " [%s]" category)
                              "")))
              (dolist (fact-entry facts)
                (let* ((fact (cdr fact-entry))
                       (key (plist-get fact :key))
                       (value (plist-get fact :value))
                       (cat (plist-get fact :category))
                       (time (plist-get fact :timestamp)))
                  (insert (format "%-20s: %s\n" key value))
                  (insert (format "  Category: %s, Updated: %s\n\n" cat time))))
              (goto-char (point-min))
              (special-mode))
            (pop-to-buffer buf))
        (message "No facts found%s"
                (if category (format " in category: %s" category) ""))))
    (mapcar #'cdr facts)))

;;;###autoload
(defun gemini-repl-memory-clear ()
  "Clear all facts from memory.
Prompts for confirmation before clearing."
  (interactive)
  (when (yes-or-no-p "Clear all facts? This cannot be undone. ")
    (setq gemini-repl-memory--facts nil)
    (when gemini-repl-memory-auto-save
      (gemini-repl-memory-save))
    (message "All facts cleared")))

;;;###autoload
(defun gemini-repl-memory-to-context ()
  "Generate system prompt context from facts.
Returns a formatted string suitable for including in LLM context."
  (gemini-repl-memory--ensure-loaded)
  (if (null gemini-repl-memory--facts)
      ""
    (let ((categories (make-hash-table)))
      ;; Group facts by category
      (dolist (fact-entry gemini-repl-memory--facts)
        (let* ((fact (cdr fact-entry))
               (category (plist-get fact :category)))
          (push fact (gethash category categories))))

      ;; Build context string
      (with-temp-buffer
        (insert "# Stored Facts\n\n")
        (insert "The following facts have been learned about the user and their environment:\n\n")

        (dolist (category gemini-repl-memory-categories)
          (when-let ((facts (gethash category categories)))
            (insert (format "## %s\n\n"
                          (capitalize (symbol-name category))))
            (dolist (fact (nreverse facts))
              (insert (format "- %s: %s\n"
                            (plist-get fact :key)
                            (plist-get fact :value))))
            (insert "\n")))

        (buffer-string)))))

;;; Persistence

(defun gemini-repl-memory-save ()
  "Save facts to disk."
  (interactive)
  (gemini-repl-memory--ensure-directory)
  (let ((facts-data (mapcar (lambda (fact-entry)
                              (let ((fact (cdr fact-entry)))
                                `((key . ,(plist-get fact :key))
                                  (value . ,(plist-get fact :value))
                                  (category . ,(symbol-name (plist-get fact :category)))
                                  (timestamp . ,(plist-get fact :timestamp)))))
                            gemini-repl-memory--facts)))
    (with-temp-file gemini-repl-memory-file
      (insert (json-encode facts-data)))
    (when (called-interactively-p 'any)
      (message "Facts saved to %s" gemini-repl-memory-file))))

(defun gemini-repl-memory-load ()
  "Load facts from disk."
  (interactive)
  (if (file-exists-p gemini-repl-memory-file)
      (condition-case err
          (progn
            (with-temp-buffer
              (insert-file-contents gemini-repl-memory-file)
              (let ((facts-data (json-read)))
                (setq gemini-repl-memory--facts
                      (mapcar (lambda (fact-obj)
                                (let ((key (alist-get 'key fact-obj))
                                      (value (alist-get 'value fact-obj))
                                      (category (intern (alist-get 'category fact-obj)))
                                      (timestamp (alist-get 'timestamp fact-obj)))
                                  (cons key
                                        (list :key key
                                              :value value
                                              :category category
                                              :timestamp timestamp))))
                              facts-data))))
            (setq gemini-repl-memory--loaded t)
            (when (called-interactively-p 'any)
              (message "Loaded %d facts from %s"
                      (length gemini-repl-memory--facts)
                      gemini-repl-memory-file)))
        (error
         (message "Error loading facts: %s" (error-message-string err))
         (setq gemini-repl-memory--facts nil
               gemini-repl-memory--loaded nil)))
    (setq gemini-repl-memory--facts nil
          gemini-repl-memory--loaded t)
    (when (called-interactively-p 'any)
      (message "No facts file found, starting fresh"))))

;;; Integration with gemini-repl

(defun gemini-repl-memory--inject-context ()
  "Inject memory context into conversation.
This function can be called before sending messages to LLM."
  (let ((context (gemini-repl-memory-to-context)))
    (when (and context (not (string-empty-p context)))
      context)))

(provide 'gemini-repl-memory)
;;; gemini-repl-memory.el ends here
