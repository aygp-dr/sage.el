...
Implemented a complete file-based queue system (`sage-queue`) for asynchronous inter-agent communication. The system enables multiple AI agents, Emacs instances, or external processes to communicate through JSON files in a watched directory structure.

** Files Created

*** Core Implementation

1. *sage-queue.el* (16.5 KB)
...
4. *test/sage-queue-test.el* (7.0 KB)
...
- `sage-queue-send-to-agent` - Direct agent messaging
- `sage-queue-broadcast` - Broadcast to all agents

*** Built-in Handlers

1. *Ping Handler*: Simple echo for testing
2. *Prompt Handler*: Executes prompts via `sage-exec`
...
sage-queue-max-retries

;; Archive retention (days)
sage-queue-retention-days
#+end_src
...
#+begin_src bash
* Submit from shell
cat > ~/.emacs.d/sage/queues/input/req.json << EOF
{"id":"shell-123","type":"ping","content":"test"}
EOF

* Read response
cat ~/.emacs.d/sage/queues/output/shell-123.json
#+end_src
...
Ready for immediate use in the sage-010 project and can serve as a foundation for complex multi-agent workflows.
