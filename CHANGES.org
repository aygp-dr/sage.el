...
Added comprehensive Emacs-native integration features to sage-010, providing deep integration with Org mode, Dired, prog-mode, and buffer operations.

** New Files

*** Core Implementation

*sage-emacs.el* (504 lines)
...
*test/sage-emacs-test.el* (90 lines)
...
Functions:
- `sage-org-send-subtree` - Send current org subtree as context
- `sage-org-send-src-block` - Send org source block to AI
...
Functions:
- `sage-explain-error` - Explain error at point with context
- `sage-suggest-fix` - Suggest fix for code at point
- `sage-explain-at-point` - Explain symbol or code at point
...
Functions:
- `sage-explain-compilation-error` - Explain compilation error
...
Modes:
- `sage-mode` - Buffer-local minor mode
- `global-sage-mode` - Global minor mode
...
Helper Functions:
- `sage--insert-as-comment` - Insert text as comment
- `sage--lang-for-mode` - Detect syntax highlighting language
- `sage-emacs-capture-response` - Capture responses for insertion
...
Advice:
- `sage--handle-response` - Capture responses automatically
...
Run tests:
#+begin_src bash
cd /home/jwalsh/ghq/github.com/aygp-dr/sage-010
emacs -batch -l ert \
  -l sage.el \
  -l sage-emacs.el \
  -l test/sage-emacs-test.el \
...
- *Lines of Code*: ~504 (sage-emacs.el)
- *Test Lines*: ~90 (test/sage-emacs-test.el)
...
- Integrates with: sage.el (required)
...
- sage-context.el - Context management
- sage-session.el - Session persistence
